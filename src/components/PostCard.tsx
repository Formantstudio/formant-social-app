...
export default function PostCard({ post, following, disablePostTap }: Props) {
  ...
  const handleFollowToggle = useCallback(async () => {
    if (!uid || !post.uid || isOwnPost) return;
    setFollowBusy(true);
    try {
  ...
  return (
    <Pressable onPress={goToPost} style={styles.card}>
      ...
      <View style={styles.header}>
        ...
        <Pressable onPress={goToAuthor} style={styles.headerText}>
      ...
      {post.type === "text" && post.content ? (
        ...
        <Pressable onPress={openVideo} style={styles.videoWrap}>
          ...
          {post.title ? (
            <Text style={styles.videoTitle} numberOfLines={2}>{post.title}</Text>
          ) : null}
        </Pressable>
      ) : null}
      ...
      <View style={styles.footer}>
        ...
        <Pressable style={styles.footerBtn} onPress={goToPost}>
